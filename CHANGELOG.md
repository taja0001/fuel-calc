...
per postcode area with ten or more stations (Bromley to BT), city-name slugs for the
unambiguous areas, generated by `scripts/build-pages.mjs` from the same prices the
Pi already pushes. Every page is real data in static HTML: the cheapest-first table
with unleaded and diesel side by side, computed area stats, the app's own name-calming
and honesty pills (members-only, price-age), and a freshness stamp that says "as of
<date>" — never "live"; the app is the live path and the CTA says so. Districts stay
deliberately unpaged (604 of them have exactly one station — the doorway-page trap).
A daily Action rebuilds the pages off the top of the hour; the Pi keeps the o'clock.
Design previewed on real Nottingham/Leeds/Halifax pages and picked before shipping.

**The app takes `?pc=` links.** Town-page CTAs arrive as `/?pc=NG18` — prefill only,
